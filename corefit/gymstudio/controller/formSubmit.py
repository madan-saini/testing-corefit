...
def basicProfile(request):
    ...
    if facility_profile_level == 'Brand':
        ...
        for e_id in equipment_arr:
            ...
            if quantity == None or quantity == '':
                quantity = 0
            if max_value == None or max_value == '' or max_value == '.':
                max_value = 0
...
def identityForm(request):
    ...
    images = User.objects.filter(id=user_id)
    context = {
        0:images.business_doc,1:images.director_doc
        }
    return JsonResponse(context)
def updateMedia(request):
    ...
    if myMedia != '':
        ...
        file_image = fss.save('static/uploads/media/'+file_name, myMedia)
        serMediaRecord= UserMediaRecord(user_id=user_id,mediaTitle=mediaTitle,type='myMedia[]',status=1,data=file_name,created_at=datetime.utcnow(),updated_at=datetime.utcnow())
        serMediaRecord.save()
...
def clientTransformation(request):
    ...
    if user_type == "Personal Trainer":
        user = User.objects.get(freelance_id=user_id)
        user_id = user.id
    serClientTransformation = UserClientTransformation(user_id=user_id, status=1,
